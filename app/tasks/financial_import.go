...
func NewFinancialImportTaskFn(store *marketdata.Store, client importer.Client) func(ctx context.Context) error {
	return func(ctx context.Context) error {
		...
		if client != nil {
			if err := backfillInstrumentPrices(ctx, store, client); err != nil {
				return err
			}
		}
...
// backfillInstrumentPrices fetches and stores recent price history for all instruments in the store.
// It skips instruments that already have a full trading-day range or data through the last trading day.
func backfillInstrumentPrices(ctx context.Context, store *marketdata.Store, client importer.Client) error {
