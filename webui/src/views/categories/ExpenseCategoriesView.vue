...
import { CreateExpenseCategoryDTO, UpdateExpenseCategoryDTO } from '@/types/category'
...
const { ExpenseTreeData } = useCategoryTree()
...
const handleAddEdit = (item: Category | null, action: string) => {
    ...
    categoryData.value.type = 'expense'
...
const saveCategory = () => {
    ...
    if (categoryData.value.action === 'add') {
        ...
        createExpenseMutation.mutate(dto, {
            ...
            onError: (err) => console.error('Failed to add expense category', err),
    ...
    if (categoryData.value.action === 'edit') {
        ...
        updateExpenseMutation.mutate(
            ...
            {
                onSuccess: () => { categoryDialogVisible.value = false },
                onError: (err) => console.error('Failed to update expense category', err),
                onSettled: () => { resetCategoryData() }
            }
        )
    }
...
const confirmDelete = (item: SelectDeleteCategory) => {
    selectedItem.value = { ...item, type: 'expense' }
...
<template>
    <div>
        <TreeTable :value="ExpenseTreeData" :expandedKeys="expandedKeys">
